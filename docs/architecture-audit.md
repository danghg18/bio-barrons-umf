...
Next recommended bounded step: extract only the shared storage helpers used by `chapter-redesign.js`, preserving their error-handling behavior and the existing entry point. Test preferences before migrating any additional consumers. Do not start routing, CSS consolidation, or the remaining metadata migrations as part of the completed extraction.
