...
(function () {
  ...
  function trapTab(event, dialog) {
    ...
    const controls = [...dialog.querySelectorAll('button:not([disabled]), input:not([disabled]), textarea:not([disabled]), select:not([disabled]), a[href], summary, [tabindex]:not([tabindex="-1"])')].filter(node => node.getClientRects().length && !node.hidden && !node.closest('details:not([open]) > :not(summary)'));
  ...
  function init() {
    if (!window.BBAuth || document.querySelector('.bb-account-surface')) return;
    const actions = document.querySelector('.lab-topbar-actions');
    if (!actions) return;
    const mount = document.getElementById('bb-account-mount');
    const inline = !!mount;
    ...
    const exportButton = panel.querySelector('#bb-cache-export');
    const backupTools = panel.querySelector('.bb-account-backup-tools');
    const urgentTools = panel.querySelector('.bb-account-urgent-tools');
    ...
    function close() { if (!inline && panel.open) panel.close(); }
    ...
    function open(nextMode) {
      ...
      if (inline) {
        (content.querySelector('input:not([disabled])') || title).focus();
        return;
      }
      ...
      (content.querySelector('input:not([disabled])') || panel.querySelector('.bb-dialog-close')).focus();
    }
    ...
    function syncStatus() {
      ...
      if (exportButton.parentElement !== exportHome) exportHome.append(exportButton);
    ...
    function switchMode(nextMode) {
      ...
      setMessage('');
      render(true);
      content.querySelector('input:not([disabled])')?.focus();
    }
