...
- The renal lesson, male reproductive lesson, and nervous-system quiz do not load `lesson.js`. They retain their local routing globals and the `chapter-redesign.js` compatibility wrapper. Renal and male pages require separate adapters for `SUB_NAVS`, `?goto`, local search, and lesson-specific interactions; the quiz requires separate handling for its generated question ranges and saved state.
