...
- `assets/js/chapter-redesign.js` is currently a compatibility controller: it wraps local `goto`, `closeNav`, highlighter toggles, and legacy search openers on non-migrated lessons. On migrated lessons it delegates search navigation to `BBLessonNavigation` and listens for its section-change event. Do not remove a local function from a legacy page without accounting for its wrappers and inline callers.
...
- Search text matching and temporary markers are shared through `BBLessonSearchText` in `chapter-redesign.js`, including the renal/male search controllers. Highlighter wrappers are transparent to matching; one result can span multiple markers. Clearing search must preserve the student's `mark.hl` elements. Legacy URL-search initialization waits for the shared script to load. Run `npm run test:search` for the search/highlighter regression suite.
- `testare.html` is the quiz catalog. Sense-organ quiz data lives in `assets/js/grile-organe-de-simt-data.js`, with the independent user answer key in `tests/organe-de-simt-answer-key.json`; its state key is `bb.quiz.organe-simt.v1`. Both quizzes share `quiz-player.js` and the DOM-discovered range router `legacy/quiz-navigation.js`. Quiz back links resolve their parent lesson through the chapter resource registry. Correct omitted choices use yellow feedback, distinct from selected correct choices (green) and extra choices (red). See `docs/grile-organe-de-simt.md` for source and verification notes.
...
- The interface is light-only. `assets/js/light-mode.js` safely retires the old `darkMode` preference. Preferences still use `highlighterMode` and `highlighterColor`; quiz state is unchanged. Highlight passages are not persisted. Do not imply otherwise.
...
- Quiz maps retain ten questions per range, with native `#grila-N` links resolving the containing section in `legacy/quiz-navigation.js`. Preserve existing range hashes and search focus suppression. `statistici.html` consumes the shared `BBQuizAnalytics` API from `quiz-analytics.js`; IndexedDB history is separate from existing quiz answers. Retrying/resetting answers never deletes history. `quiz-index.js` is generated from registered quiz datasets, never edited manually. Run `npm run test:analytics` and preserve the authored data fingerprints in `tests/quiz-content-hashes.json`. See `docs/testing-analytics.md`.
...
- For styling changes, compare desktop/phone, light-only startup with stale theme preferences, reduced motion, tables, figures, and print. Preserve computed appearance before deleting override rules.
...
## Supabase accounts (explicitly authorized extension)

- The deployed application remains static. Public configuration lives in `assets/js/supabase-config.js` and accepts Project URL + publishable key only. Never add service-role/secret keys or database passwords. The checked-in example stays empty.
- `user-storage.js` owns identity-scoped records, pending revisions, guest import and raw study/quiz compatibility mirrors. Use its API for personal progress and attempts; do not read/write another user's vault or bypass it with localStorage. Raw storage events are invalidations, not authoritative data. Retain unknown/future cloud rows.
- `auth-state.js` owns the session and emits synchronous auth events. Never await Supabase inside an SDK auth callback. `cloud-sync.js` owns hydration, serialized debounced upserts and retry; callbacks must be fenced by identity/epoch. Cloud is authoritative for clean data; durable pending same-account edits replay after hydration, with conflicts retained for export.
- `account-ui.js` owns shared account dialogs and `notes.js` owns notes keyed to chapter/active section. Never change authored lesson hashes for auth; redirect only to `cont.html`. Anonymous users can study but cannot edit notes.
- Quiz analytics histories stay local, in separate per-owner IndexedDB databases; preserve the legacy guest database and never invent attempts from cloud state.
- `npm run generate` builds the pinned local SDK and regenerates precache. `npm run test:accounts` uses an injected mock and PostgreSQL/PGlite; it must never require or mutate the real Supabase project. The live dashboard/SMTP smoke remains a separate manual check. See `docs/supabase-setup.md`.
