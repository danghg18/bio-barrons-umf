...
Read [the architecture audit](docs/architecture-audit.md) before architecture work. Its numbered sections record the original audit and proposed phased target; the migration-progress section records completed changes. Proposed files are not an instruction to perform the whole migration. Chapter-data extraction and the muscular-tissue shared-router pilot have been completed; the remaining lessons have not been migrated.
...
- `assets/js/lesson.js` is the canonical router only on lessons that explicitly load it. During the transition, `tesutul_muscular.html` is the sole migrated lesson. It loads `lesson.js` before `chapter-redesign.js`, discovers routes from `.page-section[id^="page-"]`, derives its default from the authored active section, and uses ordinary same-document hash links. Do not add local `pages`, `goto()`, `closeNav()`, or `handleHash()` routing code to a migrated lesson.
...
- Non-migrated lessons do not load `lesson.js` yet. They retain their local routing globals and the `chapter-redesign.js` compatibility wrapper. Migrate them one at a time after an individual baseline; do not bulk-add the shared script. Renal and male reproductive pages need separate handling for `SUB_NAVS`, `?goto`, local search, and lesson-specific interactions, and the quiz needs its own compatibility review.
...
- `assets/js/chapter-redesign.js` is currently a compatibility controller: it wraps local `goto`, `closeNav`, dark-mode/highlighter toggles, and legacy search openers on non-migrated lessons. On the migrated pilot it delegates search navigation to `BBLessonNavigation` and listens for its section-change event. Do not remove a local function from another lesson without accounting for its wrappers and inline callers.
